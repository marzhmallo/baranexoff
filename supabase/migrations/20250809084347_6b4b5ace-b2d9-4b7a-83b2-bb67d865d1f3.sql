-- Update accept_data_transfer to populate accepted_on when approving a transfer
...
BEGIN
    ...
    IF LOWER(transfer_record.status) != 'pending' THEN
    ...
    ELSIF transfer_record.datatype = 'documents' THEN
        ...
        SET brgyid = transfer_record.destination
        WHERE id = ANY(transfer_record.dataid);
    ELSIF transfer_record.datatype IN ('profiles', 'users') THEN
    ...
    -- Step 2: Update the transfer request status to 'Accepted', set reviewer and accepted_on timestamp

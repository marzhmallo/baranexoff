...
CREATE POLICY IF NOT EXISTS "Payme: Authenticated users can view own brgy" 
ON public.payme
FOR SELECT
USING (
  brgyid = (
...
CREATE POLICY IF NOT EXISTS "Payme: Admin/Staff insert in their brgy"
ON public.payme
FOR INSERT
WITH CHECK (
  EXISTS (
...
CREATE POLICY IF NOT EXISTS "Payme: Admin/Staff update in their brgy"
ON public.payme
FOR UPDATE
USING (
  EXISTS (
...
CREATE POLICY IF NOT EXISTS "Payme: Admin/Staff delete in their brgy"
ON public.payme
FOR DELETE
USING (
  EXISTS (

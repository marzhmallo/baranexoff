...
create policy if not exists "Docx: Users can insert own sign-up IDs"
  on public.docx
  for insert
  to authenticated
  with check (
...
create policy if not exists "Docx: Admin manage user-submitted IDs by brgy"
  on public.docx
  for all
  to authenticated
  using (
...
create policy if not exists "usersdis: Users can upload their own"
  on storage.objects
  for insert
  to authenticated
  with check (
...
create policy if not exists "usersdis: Admins can view by brgy"
  on storage.objects
  for select
  to authenticated
  using (
...
create policy if not exists "usersdis: Admins can delete by brgy"
  on storage.objects
  for delete
  to authenticated
  using (
